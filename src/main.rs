...
fn main() {
    ...
    rocket::custom({
        ...
        Config::build(Environment::Staging)
            ...
            .limits(settings.size_limits.clone().into_iter().fold(
                Limits::new(),
                |limits, (key, value)| {
                    info!(r#"Defining limit for type "{}" to be {}"#, key, value);
    ...
    .manage({
        info!(
            "Setting up {} connections to Redis server at redis://{}/{}",
            settings.workers, settings.database_address, settings.database_id
        );
        ...
        state::Server {
            db_conn: pool::Pool::generate(settings.workers as usize, |index| {
                ...
                let url = format!(
                    "redis://{}/{}",
                    settings.database_address, settings.database_id
                );
    ...
    fslogger.and_then(|(fslog, fslog_thread)| {
        ...
        fslog_thread
            .join()
            .expect("fslog thread didn't finish properly");
...
fn init_logger(
    ...
    let mut dispatch = {
        ...
        fern::Dispatch::new()
            ...
            .filter(move |meta| meta.level() <= sclone.logging.level)
            .chain(std::io::stderr())
    };
...
fn init_settings(args: &cmdargs::Arguments) -> settings::Settings {
    ...
    let data = match &args.config {
        Some(ref config) => match File::open(config) {
            Ok(mut file) => {
    ...
    toml::from_str::<settings::Settings>(&data).unwrap_or_else(|what| {
            ...
            Some(match bit.as_str() {
                s @ "-c" | s @ "--config" => {
                    if let Some(path) = self.0.next() {
                        ...
                        Err(format!(r#"expected path to config file after "{}""#, s))
                    }
                }
