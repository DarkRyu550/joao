...
impl<P: AsRef<Path> + Send + Sync> FsLogger<P> {
	pub fn new(
		...
		size_max: u64,
		history_size: u64,
		dir_path: P) -> Result<FsLogger<P>, Error> {
	...
	fn checkout(&self) -> Result<(), Error> {
		let mut lock = self.logf.lock()
			.map_err(|_| Error::OutputPoisoned)?;
		...
		let mut target = {
			...
			XzEncoder::new(
				File::create(path).map_err(iomap)?,
				COMPRESSION
			)
		};
	...
	fn flush(&self) {
		...
		let checkout = match self.logf.lock() {
			Ok(mut logf) => {
				...
				let wsize = messages.into_iter()
					.fold(Vec::new(), |mut vec, line| {
						use std::io::Write;
						let len = line.as_bytes().len();
						vec.push(
							logf.write_all(line.as_bytes())
								.map(move |_| len as u64)
						);
						vec
					})
					.into_iter()
...
impl<L: log::Log + 'static> AsyncFlusher<L> {
	pub fn new(logger: L, delay: Duration) 
