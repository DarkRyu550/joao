        ...
        Pool {
            ...
            queue: {
                ...
                (0..nelem)
                    .into_iter()
                    .skip(1)
                    .for_each(|_| queue.push(model.clone()).expect("Cannot initialize queue"));
                queue.push(model).expect("Cannot initialize queue");
    ...
    pub fn generate<F: FnMut(usize) -> T>(nelem: usize, mut generator: F) -> Pool<T> {
        ...
        Pool {
            ...
            queue: {
                ...
                (0..nelem)
                    .into_iter()
                    .for_each(|i| queue.push(generator(i)).expect("Cannot initialize queue"));
